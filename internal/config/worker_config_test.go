...
func TestRabbitmqConfig_DefaultsAndCustom(t *testing.T) {
	...
	if size != constants.DefaultRabbitmqPublishChanSize {
		t.Fatalf("expected publish chan size %d, got %d", constants.DefaultRabbitmqPublishChanSize, size)
	}
	...
	url2, size2 := rabbitmqConfig()
	expectedURL2 := fmt.Sprintf("amqp://%s:%s@%s:%s/", "u1", "p1", "rm-host", "12345")
	if url2 != expectedURL2 {
		t.Fatalf("expected url %q, got %q", expectedURL2, url2)
	}
	if size2 != 7 {
		t.Fatalf("expected publish chan size %d, got %d", 7, size2)
	}
}
...
func TestWorkerConfig_DefaultsAndCustom(t *testing.T) {
	name, mw := workerConfig()
	if name != constants.DefaultWorkerQueueName {
		t.Fatalf("expected default worker queue name %q, got %q", constants.DefaultWorkerQueueName, name)
	}
	if mw != constants.DefaultMaxWorkers {
		t.Fatalf("expected default max workers %d, got %d", constants.DefaultMaxWorkers, mw)
	}

	t.Setenv("WORKER_QUEUE_NAME", "custom_queue")
	t.Setenv("MAX_WORKERS", "3")
	name2, mw2 := workerConfig()
	if name2 != "custom_queue" {
		t.Fatalf("expected worker queue name %q, got %q", "custom_queue", name2)
	}
	if mw2 != 3 {
		t.Fatalf("expected max workers %d, got %d", 3, mw2)
	}
}
